    ...
    <div class="page no-break">
        ...
        <div class="info">
            <div class="info-row">
                ...
                <div class="info-cell">
                    <p><strong>Mã toa thuốc:</strong> {{ $medical_record_code ?? ($code ?? 'TT') }}</p>
                    <p><strong>Ngày khám:</strong> {{ $appointment_date ?? ($date ?? date('d/m/Y')) }}</p>
        ...
        <!-- 🔥 PHẦN THUỐC VỚI REAL-TIME SETTINGS -->
        @php
        ...
        @if($hasMedicines)
                ...
                <tbody>
                    @foreach ($prescriptions as $index => $medicine)
                        @php
                            ...
                            $medicineName = $medicine['MedicineName'] ?? $medicine['medicine_name'] ?? 'N/A';
                            $usage = $medicine['Usage'] ?? $medicine['dosage'] ?? 'Theo chỉ định của bác sĩ';
                        @endphp
                        <tr>
            ...
            @if($totalMedicineAmount > 0)
                <div class="money-in-words">
                    <span class="money-label">Số tiền viết bằng chữ:</span>
                    <span class="money-words">
                        @php
                            function numberToWordsMedicine($number)
                            {
                                ...
                                if ($number < 10) {
                                    ...
                                    return $ones[floor($number / 100)] . " trăm" . ($number % 100 != 0 ? " " . numberToWordsMedicine($number % 100) : "");
                                } elseif ($number < 1000000) {
                                    return numberToWordsMedicine(floor($number / 1000)) . " nghìn" . ($number % 1000 != 0 ? " " . numberToWordsMedicine($number % 1000) : "");
                                } elseif ($number < 1000000000) {
                                    return numberToWordsMedicine(floor($number / 1000000)) . " triệu" . ($number % 1000000 != 0 ? " " . numberToWordsMedicine($number % 1000000) : "");
                                } else {
                                    return numberToWordsMedicine(floor($number / 1000000000)) . " tỷ" . ($number % 1000000000 != 0 ? " " . numberToWordsMedicine($number % 1000000000) : "");
                                }
                            }
                        @endphp
                        {{ ucfirst(numberToWordsMedicine($totalMedicineAmount)) }} đồng
                    </span>
                </div>
            @endif
        ...
        <!-- 🔥 HƯỚNG DẪN SỬ DỤNG VỚI REAL-TIME FONTS -->
