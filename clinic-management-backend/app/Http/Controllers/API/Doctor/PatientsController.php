...
class PatientsController extends Controller
{
    ...
    public function getPatientHistory($patientId)
    {
        try {
            ...
            // Lấy thông tin cuộc hẹn kèm thông tin bác sĩ
            $appointment = Appointment::with(['medical_staff.user'])
                ->where('PatientId', $patientId)
                ->first();

            $doctorName = 'Chưa có bác sĩ'; // Giá trị mặc định

            if ($appointment && $appointment->medical_staff) {
                $doctorInfo = [
                    'doctor_id' => $appointment->medical_staff->StaffId,
                    'doctor_name' => $appointment->medical_staff->user->FullName,
                    'specialization' => $appointment->medical_staff->Specialization,
                    'phone' => $appointment->medical_staff->user->Phone,
                    'email' => $appointment->medical_staff->user->Email
                ];
                $doctorName = $doctorInfo['doctor_name']; // Lấy tên bác sĩ từ array
            }
