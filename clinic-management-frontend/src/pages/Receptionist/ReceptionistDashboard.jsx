...
const ReceptionistDashboard = () => {
    ...
    const fetchQueueByRoom = async (roomId) => {
        setLoading(true);
        try {
            const response = await axiosInstance.get(`/api/receptionist/queue/${roomId}`);
    ...
    const filteredQueue = queue
        .filter(item => {
            ...
            const searchMatch = item.PatientName?.toLowerCase().includes(searchTerm.toLowerCase());
            return roomMatch && searchMatch;
        })
        .sort((a, b) => {
            // Sort by status priority: Đang khám -> Đang chờ -> Đã khám
            const statusOrder = { "Đang khám": 1, "Đang chờ": 2, "Đã khám": 3, "Hủy": 4 };
            return statusOrder[a.Status] - statusOrder[b.Status] || a.QueuePosition - b.QueuePosition;
        });
    ...
    const handleCallPatient = async (queueId) => {
        try {
            ...
            showNotification(`Đã gọi bệnh nhân ${patient.PatientName} vào khám`, "success");
        } catch (error) {
    ...
    const confirmCancelAppointment = async () => {
        if (deleteModal.patient) {
            try {
                await axiosInstance.put(`/api/receptionist/queue/${deleteModal.patient.queueId}/status`, {
                    Status: "Hủy"
                });

                setQueue(prevQueue => prevQueue.filter(item => item.QueueId !== deleteModal.patient.queueId));
                showNotification(`Đã hủy lịch khám của ${deleteModal.patient.PatientName}`, "warning");
            } catch (error) {
    ...
    const handleCompleteAppointment = async (queueId) => {
        try {
            ...
            const patient = queue.find(p => p.QueueId === queueId);
            showNotification(`Đã hoàn tất khám cho ${patient.PatientName}`, "success");
        } catch (error) {
    ...
    const renderStatus = (status) => {
        ...
        const statusInfo = statusMap[status] || { class: "bg-secondary text-white", icon: "bi-question", label: status };

        return (
            <span className={`badge ${statusInfo.class} d-flex align-items-center justify-content-center`} style={{ minWidth: '100px' }}>
    ...
    const priorityCount = filteredQueue.filter(item => item.QueuePosition === 1 && item.Status === "Đang chờ").length;
            ...
            {notification.show && (
                <div className={`alert alert-${notification.type} alert-dismissible fade show position-fixed top-0 end-0 m-3`}
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                        <span>
                            <i className={`bi ${notification.type === 'success' ? 'bi-check-circle' : notification.type === 'warning' ? 'bi-exclamation-triangle' : 'bi-info-circle'} me-2`}></i>
            ...
            <div className="card shadow-sm border-0">
                <div className="card-body p-4">
                    ...
                    <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
                        <div>
                            <h4 className="fw-bold text-primary mb-1">Quản lý hàng chờ khám bệnh</h4>
                        ...
                        <div className="mt-2 mt-md-0">
                            <div className="d-flex flex-wrap gap-2">
                                <div className="d-flex align-items-center bg-light rounded px-3 py-2">
                                    <span className="text-primary fw-bold me-1">{filteredQueue.length}</span>
                                    <span className="text-muted">Tổng số</span>
                                </div>
                                <div className="d-flex align-items-center bg-light rounded px-3 py-2">
                                    <span className="text-warning fw-bold me-1">{countByStatus("Đang chờ")}</span>
                                    <span className="text-muted">Đang chờ</span>
                                </div>
                                <div className="d-flex align-items-center bg-light rounded px-3 py-2">
                                    <span className="text-danger fw-bold me-1">{priorityCount}</span>
                                    <span className="text-muted">Ưu tiên</span>
                                </div>
                                <div className="d-flex align-items-center bg-light rounded px-3 py-2">
                                    <span className="text-success fw-bold me-1">{countByStatus("Đã khám")}</span>
                    ...
                    <div className="row mb-4">
                        ...
                        <div className="col-md-6">
                            <Select
                                ...
                                value={
                                    selectedRoom
                                        ? { value: selectedRoom.RoomId, label: selectedRoom.RoomName }
                                        : { value: null, label: "Tất cả phòng" }
                                }
                                isSearchable
                            ...
                            <table className="table table-hover align-middle clinic-table mb-0">
                                <thead className="table-light">
                                    <tr>
                                        ...
                                        <th>Tên Bệnh Nhân</th>
                                        <th className="text-center" style={{ width: '100px' }}>Giờ Đặt</th>
                                        <th>Bác Sĩ</th>
                                        <th className="text-center" style={{ width: '150px' }}>Phòng</th>
                                ...
                                <tbody>
                                    {filteredQueue.length > 0 ? (
                                        filteredQueue.map((item, index) => (
                                            <tr key={item.QueueId} className={`table-row-hover ${item.Status === "Đang khám" ? "table-active" : ""} ${item.QueuePosition === 1 && item.Status === "Đang chờ" ? "border-start border-danger border-3" : ""}`}>
                                                <td className="text-center">
                                                    {canPrioritize(item.Status) && (
                                                        <button
                                                            className={`btn btn-sm p-0 ${item.QueuePosition === 1 ? 'text-warning' : 'text-muted opacity-50'}`}
                                                            onClick={() => handleTogglePriority(item.QueueId)}
                                                            title={item.QueuePosition === 1 ? "Đã ưu tiên" : "Đánh dấu ưu tiên"}
                                                            disabled={item.QueuePosition === 1}
                                                        >
                                                            <i className={`bi ${item.QueuePosition === 1 ? 'bi-star-fill' : 'bi-star'}`} style={{ fontSize: '1.2rem' }}></i>
                                                        </button>
                                                    )}
                                                </td>
                                                <td className="text-center">
                                                    <div className="fw-bold text-primary">{item.QueuePosition || index + 1}</div>
                                                </td>
                                                <td>
                                                    <div className="d-flex align-items-center">
                                                        <div className="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3"
                                                            style={{ width: '40px', height: '40px', fontSize: '14px', fontWeight: 'bold' }}>
                                                            {item.PatientName?.split(' ').pop()?.charAt(0) || 'P'}
                                                        </div>
                                                        <div className="flex-grow-1">
                                                            <div className="fw-semibold">{item.PatientName}</div>
                                                            <small className="text-muted">ID: {item.PatientId}</small>
                                                        </div>
                                                ...
                                                <td className="text-center">
                                                    <span className="fw-bold text-primary bg-light px-2 py-1 rounded">
                                                        {item.QueueTime ? item.QueueTime.substring(0, 5) : '--:--'}
                                                    </span>
                                                </td>
                                                <td className="fw-medium">{item.doctor}</td>
                                                <td className="text-center">
                                                    <span className={`badge bg-${item.color}-soft px-3 py-2`}>
                                                    ...
                                                    <div className="d-flex gap-2 justify-content-center flex-wrap">
                                                        {item.Status === "Đang chờ" && (
                                                            <>
                                                                ...
                                                                >
                                                                    <i className="bi bi-telephone me-1"></i> Gọi khám
                                                                </button>
                                                                <button
                                                        ...
                                                        {item.Status === "Đang khám" && (
                                                            <button
                                                                ...
                                                                onClick={() => handleCompleteAppointment(item.QueueId)}
                                                            >
                                                                <i className="bi bi-check-circle me-1"></i> Hoàn tất
                                                            </button>
                                                        )}
                                                        {item.Status === "Đã khám" && (
                                                            <button className="btn btn-sm btn-outline-success d-flex align-items-center" disabled>
                                                                <i className="bi bi-check2-all me-1"></i> Đã khám
                                                            </button>
                                                        )}
                                                        {item.Status === "Hủy" && (
                                                            <button className="btn btn-sm btn-outline-secondary d-flex align-items-center" disabled>
                                        ...
                                        <tr>
                                            <td colSpan="8" className="text-center py-5">
                                                <div className="text-muted">
                                                    ...
                                                    {selectedRoom ? 'Không có bệnh nhân nào trong phòng này.' : 'Không tìm thấy bệnh nhân nào.'}
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                    ...
                    <div className="row align-items-center">
                        <div className="col-md-6 mb-2 mb-md-0">
                            <div className="d-flex align-items-center justify-content-center justify-content-md-start">
                                ...
                                <small className="text-muted fw-semibold">Hướng dẫn sử dụng:</small>
                            </div>
                        </div>
                        <div className="col-md-6">
                            <div className="d-flex flex-wrap gap-3 justify-content-center justify-content-md-end">
                                <span className="d-flex align-items-center">
                                    ...
                                    <small className="text-muted">Nhấn ngôi sao để ưu tiên lên đầu hàng chờ</small>
                                </span>
                                <span className="d-flex align-items-center">
                                    <div className="border-start border-danger border-3 me-2" style={{ height: '16px' }}></div>
                                    <small className="text-muted">Viền đỏ = bệnh nhân ưu tiên (vị trí đầu)</small>
                                </span>
                            </div>
                        </div>
                    </div>
