...
const TodaySection = ({
  ...
  const handleError = useCallback((error, customMessage = '') => {
    ...
    // ✅ SỬ DỤNG TOAST TỪ PROPS
    showToast(customMessage || translatedError, "danger");
  }, [translateError, showToast]);

  // HÀM HIỂN THỊ CONFIRMATION VỚI XỬ LÝ LỖI TỐT HƠN
  const showConfirmation = useCallback(async (options) => {
    try {
      // THÊM: Đảm bảo Swal sẵn sàng
      if (typeof Swal === 'undefined') {
        throw new Error('SweetAlert2 chưa được tải');
      }
      ...
      // FALLBACK: dùng confirm thường nếu Swal lỗi
      const confirmed = window.confirm(
        `${options.title || 'Xác nhận hành động'}\n${options.message || 'Bạn có chắc muốn thực hiện?'}`
      );
  ...
  const fetchTodayPatients = useCallback(async () => {
    try {
      ...
      if (response && response.data) {
        ...
        setTodayPatients([]);
        showToast('Không có dữ liệu bệnh nhân hôm nay', "info");
      }
    } catch (error) {
  ...
  const loadCompletedExam = useCallback(async (appointmentId) => {
    ...
    if (!appointmentId) {
      ...
      showToast('Không tìm thấy ID cuộc hẹn', "danger");
      return;
    }
    ...
    try {
      ...
      showToast('Đã tải hồ sơ cũ để xem.', "success");
  ...
  const handleExaminationSubmit = async (e) => {
    ...
    if (!selectedTodayPatient) {
      showToast("Chưa chọn bệnh nhân.", "warning");
      return;
    }

    if (!symptoms && !diagnosis && Object.keys(services).length === 0 && prescriptionRows.length === 0) {
      showToast("Chưa có dữ liệu nào để lưu. Vui lòng nhập chẩn đoán hoặc chọn dịch vụ/thuốc.", "warning");
      return;
    }
    ...
    try {
      ...
      showToast(error.message || 'Đã xảy ra lỗi khi hoàn tất khám', "danger");
    }
  };
  ...
  const handleTempSave = async () => {
    ...
    if (!symptoms && !diagnosis && Object.keys(services).length === 0 && prescriptionRows.length === 0) {
      showToast("Chưa có dữ liệu nào để tạm lưu.", "info");
      return;
    }
  ...
  const handleRefreshPatients = async () => {
    ...
    if (result.isConfirmed) {
      await fetchTodayPatients();
      showToast('Đã làm mới danh sách bệnh nhân', "success");
    }
  };
  ...
  const handleRemoveWithConfirm = async (index) => {
    ...
    if (result.isConfirmed) {
      ...
      showToast('Đã xóa thuốc khỏi đơn', "success");
    }
  };
