...
const AdminSuppliers = () => {
  ...
  const handleDelete = useCallback(async (supplier) => {
    try {
      ...
      showToast('error', err.response?.data?.message || 'Không thể xóa (có thể đang có phiếu nhập kho)');
    } finally {
      setIsLoading(false);
  ...
  const handleEditSupplier = async (formData) => {
    const data = {
      ...
      Address: formData.get('Address')?.trim() || null,
      Description: formData.get('Description')?.trim() || null,
      version: formData.get('version'), // ← Gửi version
    };

    try {
      setIsLoading(true);
      await supplierService.update(editSupplier.SupplierId, data);
      ...
      if (err.response?.status === 409) {
        showToast('error', 'Dữ liệu đã bị thay đổi bởi người khác. Đang tải lại...');
        cache.current.clear();
        fetchSuppliers(currentPage + 1, filterParams);
      } else {
  ...
  const handleShowEditForm = useCallback((supplier = null) => {
  ...
  const renderFormModal = () => {
    ...
    const handleFormSubmit = (e) => {
      ...
      const { errors: newErrors, valid } = validateForm(fd);
    ...
    const modalLayout = (title, body) => (
      <>
        ...
        <div className="modal fade show d-block" tabIndex="-1" onClick={handleCancelForm}>
          <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '700px' }} onClick={e => e.stopPropagation()}>
    ...
    return modalLayout(
      ...
      <form onSubmit={handleFormSubmit}>
        <div className="row g-3">
          {isEditMode && (
            <input type="hidden" name="version" value={editSupplier?.version || 0} />
          )}
          ...
          <div className="col-12">
            <div className="mb-3">
              ...
              <input
                type="text"
                name="SupplierName"
                className={`form-control ${formErrors.SupplierName ? 'is-invalid' : ''}`}
                defaultValue={isEditMode ? editSupplier?.SupplierName : ''}
                placeholder="Nhập tên nhà cung cấp..."
                required
              />
              {formErrors.SupplierName && <div className="invalid-feedback">{formErrors.SupplierName}</div>}
            </div>
          </div>
  ...
  return (
    <div className="d-flex">
      <main className="main-content flex-grow-1 p-4 d-flex flex-column gap-4">
        {toast.show && <CustomToast type={toast.type} message={toast.message} onClose={() => setToast({ show: false })} />}
              ...
              <div className="card-body p-4">
                <div className="row g-3">
                  ...
                  <div className="col-md-3">
                    <input type="text" className="form-control" placeholder="Lọc theo email..." value={filters.email} onChange={(e) => setFilters(prev => ({ ...prev, email: e.target.value }))} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
                  </div>
                  <div className="col-md-3">
                    <input type="text" className="form-control" placeholder="Lọc theo số điện thoại..." value={filters.phone} onChange={(e) => setFilters(prev => ({ ...prev, phone: e.target.value }))} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
                  </div>
                  <div className="col-md-2 d-flex gap-2">
                    <button className="btn btn-primary w-100 d-flex align-items-center justify-content-center gap-2" onClick={applyFilters}>
                      <BiFilter size={16} /> Lọc
                    </button>
                    <button className="btn btn-outline-secondary w-100" onClick={clearFilters}>Xóa lọc</button>
                  </div>
                </div>
              </div>
            </div>
              ...
              {isLoading ? <Loading isLoading={isLoading} /> : (
                <>
                  <div className="d-flex justify-content-between align-items-center p-4 border-bottom">
                    <h6 className="mb-0 text-muted">Tổng cộng: <strong>{totalItems}</strong> nhà cung cấp</h6>
                    <small className="text-muted">Trang {currentPage + 1} / {pageCount}</small>
                  </div>
                      ...
                      <tbody>
                        {suppliers.length > 0 ? suppliers.map(supplier => (
                          <tr key={supplier.SupplierId}>
                            ...
                            <td><div className="text-truncate" style={{ maxWidth: '150px' }} title={supplier.Address}>{supplier.Address || '—'}</div></td>
                            <td><div className="text-truncate" style={{ maxWidth: '200px' }} title={supplier.Description}>{supplier.Description || '—'}</div></td>
                            <td className="text-center px-4">
                              <div className="d-flex gap-2 justify-content-center">
                                <button className="btn btn-lg btn-light" title="Sửa" onClick={() => handleShowEditForm(supplier)}>
                                  <BiPencil />
                                </button>
                                <button className="btn btn-lg btn-light text-danger" title="Xóa" onClick={() => handleShowDeleteModal(supplier)}>
                                  <BiTrash />
                                </button>
                              </div>
                            </td>
